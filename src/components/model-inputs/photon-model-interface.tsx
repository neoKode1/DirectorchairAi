...
export interface PhotonModelInput {
  prompt: string;
...
interface PhotonModelInterfaceProps {
  modelInfo: ApiInfo;
  onSubmit: (input: PhotonModelInput) => Promise<{ images: { url: string }[] }>;
}
...
export const PhotonModelInterface: React.FC<PhotonModelInterfaceProps> = ({ modelInfo, onSubmit }) => {
  const { toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);
  
  const [formData, setFormData] = useState<PhotonModelInput>({
    prompt: "",
    negative_prompt: "",
    num_images: 1,
...
export default PhotonModelInterface; 