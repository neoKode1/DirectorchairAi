...
export interface FluxProUltraFinetunedInput {
  prompt: string;
...
interface FluxProUltraFinetunedInterfaceProps {
  modelInfo: ApiInfo;
  onSubmit: (result: FluxProUltraFinetunedInput) => void;
}
...
> = ({ modelInfo, onSubmit }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.finetune_id) {
      toast({
        title: "Error",
        description: "Finetune ID is required",
        variant: "destructive",
      });
      return false;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <Label htmlFor="finetune_id">Finetune ID</Label>
        <Input
          id="finetune_id"
      ...
      <div>
        <Label htmlFor="finetune_strength">Finetune Strength</Label>
        <Slider
