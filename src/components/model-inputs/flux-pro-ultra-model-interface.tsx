...
export interface FluxProUltraModelInput {
  prompt: string;
...
interface FluxProUltraModelInterfaceProps {
  modelInfo: ApiInfo;
  onSubmit: (input: FluxProUltraModelInput) => Promise<{ images: { url: string }[] }>;
}
...
export const FluxProUltraModelInterface: React.FC<FluxProUltraModelInterfaceProps> = ({ modelInfo, onSubmit }) => {
  const { toast } = useToast();
  const [isLoading, setIsLoading] = useState(false);
  
  const [formData, setFormData] = useState<FluxProUltraModelInput>({
    prompt: "",
    negative_prompt: "",
    num_images: 1,
  ...
  useEffect(() => {
    console.log("[FluxProUltra] Component mounted with initial state:", formData);
    console.log("[FluxProUltra] Model info:", modelInfo);
  }, []);

  useEffect(() => {
    console.log("[FluxProUltra] Form data updated:", formData);
  }, [formData]);

  const validateForm = (): boolean => {
    console.log("[FluxProUltra] Validating form data:", formData);

    if (!formData.prompt) {
      console.warn("[FluxProUltra] Validation failed: Prompt is required");
      toast({
        title: "Error",
    ...
    if (formData.num_images && (formData.num_images < 1 || formData.num_images > 4)) {
      console.warn("[FluxProUltra] Validation failed: Invalid number of images:", formData.num_images);
      toast({
        title: "Error",
    ...
    if (formData.use_seed && ((formData.seed ?? 0) < 0 || (formData.seed ?? 0) > 2147483647)) {
      console.warn("[FluxProUltra] Validation failed: Invalid seed value:", formData.seed);
      toast({
        title: "Error",
    ...
    console.log("[FluxProUltra] Form validation successful");
    return true;
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[FluxProUltra] Form submission started");
    
    if (!validateForm()) {
      console.warn("[FluxProUltra] Form submission aborted due to validation failure");
      return;
    }

    try {
      console.log("[FluxProUltra] Setting loading state");
      setIsLoading(true);
      
      console.log("[FluxProUltra] Submitting form data to API:", formData);
      const result = await onSubmit(formData);
      console.log("[FluxProUltra] API response received:", result);
      ...
      console.error("[FluxProUltra] Error during form submission:", error);
      toast({
        title: "Error",
      ...
      console.log("[FluxProUltra] Resetting loading state");
      setIsLoading(false);
    }
  };

  const handleInputChange = (field: keyof FluxProUltraModelInput, value: any) => {
    console.log(`[FluxProUltra] Input change - Field: ${field}, Value:`, value);
...
export default FluxProUltraModelInterface; 