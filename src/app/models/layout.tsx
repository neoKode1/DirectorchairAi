...
function groupEndpointsByCategory() {
  return AVAILABLE_ENDPOINTS.reduce((acc, endpoint) => {
...
function formatModelName(endpoint: typeof AVAILABLE_ENDPOINTS[0]) {
  ...
  if (endpoint.endpointId === "Photon" || endpoint.endpointId === "Photon-Instant") {
    return `Nano • ${name}`;
  }
  if (endpoint.endpointId.includes('minimax')) {
    return `Nano • Minimax ${name.replace('Minimax ', '')}`;
  }
      ...
      <div className="w-64 border-r bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="space-y-4 py-4">
          <div className="px-3 py-2">
            <h2 className="mb-2 px-4 text-lg font-semibold">Models</h2>
            <div className="space-y-1">
              {categoryOrder.map(category => (
                categories[category] && (
                  <div key={category} className="mb-6">
                    <h3 className="px-4 text-sm font-medium text-muted-foreground mb-2 uppercase">
