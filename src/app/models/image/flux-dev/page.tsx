...
import { FluxDevModelInterface, FluxDevModelInput } from "@/components/model-inputs/flux-dev-model-interface";
...
export default function FluxDevPage() {
  ...
  const handleSubmit = async (input: FluxDevModelInput): Promise<{ images: { url: string }[] }> => {
    try {
      setIsLoading(true);
      const response = await fetch("/api/generate/flux-dev", {
        method: "POST",
        headers: {
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <FluxDevModelInterface modelInfo={modelInfo} onSubmit={handleSubmit} />
        </CardContent>
      </Card>
